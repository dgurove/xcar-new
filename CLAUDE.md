...
галки листов Сводка / Все / С предложениями / Без предложений (матрица
машина × менеджер с колонками «Максимальная», «Минимальная»), кнопки
Excel и PDF (`OffersExport::tables/xlsx/pdf`, PDF рисует dompdf по
`admin/purchases/offers-pdf`, `GET …/predlozheniya/fayl?sheets[]&format`);
файлы наружу через `file_controller` — на телефоне системный лист
