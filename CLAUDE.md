...
- **Три вида списка** (`?vid=grid|list|table`, `Support\ListView`, память
  ...
  **Окошко строки** (`peek_controller`, только в таблице): нажатие по строке —
  ...
  Телефон — лист от низа поверх таб-бара, коротко (полэкрана) или во весь экран за ручку
  (положение помнится в localStorage), стрелки у счётчика — по строкам; от 1024 —
  панель справа от таблицы, липкая. Характеристики — метками и фактами, отдельного списка нет.
  ...
  стоянка — принять, переставить, выдать, акты, заметка. Переключение строк без
  скачка: прошлое содержимое притушено (`aria-busy`), высота — переходом.
