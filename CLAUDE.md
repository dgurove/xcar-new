...
- Чаты: чат — пара (оффер, участник) + вторая сторона: `manager_id` пуст — сотрудники
  площадки (CRM «Чаты»), задан — менеджер покупателя (кабинет `/account/chats/{id}`, в CRM
  таких чатов нет). Одна дверь `Chat::isCounterpart($user)` для счётчиков, «моих» сообщений,
  доступа, live-тем и уведомлений; `User::canChat()` — покупателю только с менеджером.
