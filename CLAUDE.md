...
- **Три вида списка** (`?vid=grid|list|table`, `Support\ListView`, память
  ...
  избранное, машины закупки (`x-purchase.site-table-row`, `purchase-cars`). Переключатель `x-ui.view-switch` — круглая кнопка с иконкой
  текущего вида, меню `x-ui`-`.menu` (`menu_controller`, popover). Нажатие по
  строке — выделение и парящее окошко `.peek` (`peek_controller`, фрейм
  `peek` с `x-ui.peek`: фото, метки, факты чипами, цена, «Открыть»;
  эндпоинты `/offers/{n}/peek`, `/purchases/{n}/{car}/peek` на обоих хостах,
  `/cars/{id}/peek` на стоянке),
  двойное нажатие или Enter — на страницу, ↑/↓ — по строкам. «Сколько
