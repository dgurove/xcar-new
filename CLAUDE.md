...
- Вид — язык витрины xcar.ru, один на витрину, кабинет, админку и стоянку:
  ...
  соседям обращённую к нему сторону), в CRM и на стоянке слева от логотипа
  слово CRM/PARK шрифтом Anton и нет поиска на телефоне, нижний таб-бар
...
- CRM и стоянка плотнее витрины: `<body class="surface-crm|park">` включает
  ...
  22rem, внизу во всю ширину `x-ui.photos` (полоса фото: глаз · поворот ·
  корзина, главное — первое, порядок перетаскиванием, клик — Viewer.js с
  теми же кнопками; поворот — `RotatePhoto` на месте вместе с конверсиями) и
  документы через `x-ui.file` (квадрат с расширением). Без фото — `x-ui.car-blank`.
  Плитки-числа — `x-ui.stat`. VIN: `App\Cars\Vin` (перенос из старого проекта,
  данные в `resources/vin`), `/spravochnik/vin` и `vin_controller` заполняют
  пустые поля формы по семнадцати знакам. Цифры табличные (`.nums`) только у
