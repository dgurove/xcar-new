...
`/prodlit`); действия и обе выгрузки — в шторке «···», плиток-чисел нет: `Export` поставщику (xlsx, в «Предложение клиента» — `price_final`) и предложения формой —
галки Сводка / С предложениями / Без предложений, кнопки Excel и PDF
(`OffersExport`): Excel — **тот самый файл Carcade с диска** (`source_file`,
все листы и колонки как есть, включая пустую синюю «Предложение клиента»
под финальную цену владельца), на листе с «ДЛ» справа «Максимальная»,
«Минимальная» и колонка на менеджера, строки по галкам, умная таблица
снимается (диапазон не сходится после удаления строк), вместо неё
автофильтр; PhpSpreadsheet только здесь. PDF — компактная матрица
(dompdf по `admin/purchases/offers-pdf`). `GET …/predlozheniya/fayl?parts[]&format[&inline]`;
