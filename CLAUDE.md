...
выбрать/отменить; под тулбаром выбор состояния `x-ui.choose` «Все машины ▾»
— пресеты `PRESETS` группами `PRESET_GROUPS` (десктоп select, телефон
шторка), рядом пилюли типов техники `?kind=`; сочетаются, числа
перекрёстные; сортировка «Наша цена») и «По менеджерам» (пилюли-люди,
