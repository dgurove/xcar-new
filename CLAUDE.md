...
с галками С предложениями / Без предложений / Цены менеджеров и
...
«идёт» машина не застревает; дозабор — только `purchases:recheck` в
05:10 (`ImportFile::recheck`: пустые в момент выкачки папки облака,
`failed`/`partial` у фото и характеристик по живым закупкам, не свежее 12 ч,
до 200 машин за проход) и точечно на странице машины («Перечитать»,
«Забрать все из облака»); кнопок «перечитать всю закупку» нет.
