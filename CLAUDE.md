...
«По машинам» последним — поле **«Наша цена»** `price_final`: сохраняется само
на уходе с поля (`price_controller`, `PUT …/{car}/cena`, галочка в поле),
Enter/«Далее» — к следующей машине, плейсхолдер — максимум менеджеров; то
же поле на странице машины). «Наша цена» — что уходит поставщику,
«Выбрана» у менеджера — кому уходит машина у нас; они не связаны.
