...
- **Три вида списка** (`?vid=grid|list|table`, `Support\ListView`, память
  ...
  `ListView::pick`; страницы — `x-ui.pager` вместо `links()`: номера, «По 24 / 48 / 96» карточкам
  и «По 50 / 100 / 200» таблице и плотным спискам (`ListView::paginate` карточным, `perPage(…, PER_ROWS)`
  остальным); тулбар `x-ui.toolbar` на телефоне в две строки — пилюли на месте h1 там, где раздел
  есть в таб-баре (`x-ui.shell :phone-heading="false"`), кнопки ниже; первый столбец таблицы — номер: № предложения, ДЛ,
