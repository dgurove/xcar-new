...
оценки** `…/{car}/ocenka`: поле «Наша цена» `price_final` и «Дальше»
сразу под заголовком (не в action-bar — на iPhone полоса внизу уходит под
клавиатуру, у цифровой клавиатуры нет «Далее»), опорные цены и
предложения чипами, галерея, характеристики (`Car::factsList()`, общий
с витриной); стрелки в шапке — соседи среди машин без нашей цены по
