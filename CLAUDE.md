...
- Чаты — мессенджер по образцу Авито: чат — пара (оффер, участник) + вторая сторона:
  ...
  менеджером. Экран `/account/chats` у всех: список строками (собеседник, ТС, последнее, бейдж),
  чат — `x-chat.screen` (полоса собеседника с «в сети» по `users.seen_at`/`TouchSeen`, плашка ТС,
  лента во всю высоту, поле над клавиатурой; таб-бар и пилюли скрыты через `body:has(.chat-page)`),
  от 1024 — список слева и фрейм `chat-screen` справа (`split_controller`). Лента `chat/messages` +
