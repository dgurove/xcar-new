...
class MoneyController
{
    ...
    public function index(Request $request)
    {
        ...
        $q = Invoice::with(['party', 'vehicle.brand', 'vehicle.model'])
            ->when($request->query('party'), fn ($w, $id) => $w->where('party_id', $id))
            ->when($request->query('mashina'), fn ($w, $id) => $w->where('vehicle_id', $id));
        match ($preset) {
        ...
        $open = Invoice::where('state', InvoiceState::Issued)->get();
