...
/** Оплата частями; не больше остатка; при нуле остатка счёт оплачен. Заявка менеджера (`$claim`) становится этой оплатой, а не второй строкой. */
final class RecordPayment
{
    public function __invoke(Invoice $invoice, User $by, float $amount, ?CarbonInterface $at = null, PaymentSource $source = PaymentSource::Bank, ?string $ref = null, ?string $note = null, ?Payment $claim = null): Payment
    {
        ...
        $payment = DB::transaction(function () use ($invoice, $by, $amount, $at, $source, $ref, $note, $claim) {
            ...
            $row = ['invoice_id' => $invoice->id, 'party_id' => $invoice->party_id, 'amount' => $amount, 'paid_at' => Carbon::instance($at ?? now())->toDateString(), 'source' => $source, 'ref' => $ref, 'note' => $note, 'state' => PaymentState::Confirmed];
            if ($claim) {
                $claim->update($row + ['decided_at' => now()]);
                $payment = $claim;
            } else {
                $payment = Payment::create($row + ['created_by' => $by->id]);
            }
