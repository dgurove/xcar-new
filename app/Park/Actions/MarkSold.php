 ...
 * Тут же заявка на выдачу с контактом покупателя — она и попадает в «Выдать» на «Сегодня». Повтор — правка полей.
 */
final class MarkSold
{
    public function __construct(private CreateRequest $create) {}

    public function __invoke(Vehicle $vehicle, ?User $by, CarbonInterface $soldAt, ?string $name, ?string $phone, ?string $note = null, ?Message $message = null): Vehicle
    {
        $fresh = ! $vehicle->sold_at;
        Nav::forgetStaffCounts();
        DB::transaction(function () use ($vehicle, $by, $soldAt, $name, $phone, $note, $message, $fresh) {
            $vehicle->update(array_filter([
                'sold_at' => $soldAt->toDateString(), 'sold_message_id' => $message?->id,
                'pickup_name' => $name, 'pickup_phone' => $phone, 'pickup_note' => $note,
            ], fn ($v) => $v !== null) + ($fresh ? [] : ['pickup_name' => $name, 'pickup_phone' => $phone, 'pickup_note' => $note]));
            if ($vehicle->buyerParty && $name) {
                $vehicle->buyerParty->update(['name' => $name, 'phone' => $phone, 'notes' => $note]);
            }
            ...
            if ($fresh && $vehicle->state === VehicleState::Stored && ! $vehicle->openRequest(RequestType::Release)) {
                ($this->create)($by, RequestType::Release, $vehicle,
                    ['contact_name' => $name, 'contact_phone' => $phone, 'thread_id' => $message?->thread_id, 'note' => $note]);
            }
        });
        if ($fresh) {
