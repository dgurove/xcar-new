...
enum Category: string
{
    ...
    public static function guess(?string $text): ?self
    {
        ...
        return match (true) {
            $has('длинномер', 'фура', 'автопоезд') => self::Long,
            // Полуприцеп и марки полуприцепов — прицеп (900 ₽ по прайсу Альфы), а тягач остаётся длинномером (800 ₽).
            $has('прицеп', 'тонар', 'кроне', 'krone', 'schmitz', 'шмитц') => self::Trailer,
