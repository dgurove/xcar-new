...
enum Intent: string
{
    ...
    public static function ofOutgoing(?string $subject, ?string $body, array $filenames = []): self
    {
        ...
        if (preg_match('/(?:ТС|ГОТС|машин[ау]|автомобиль)\s+(?:выдали|выдан[аоы]?|забрали|вывезли)\b|покупатель\s+(?:забрал|вывез)\b/iu', $text)) {
            return self::Released;
        }
        if (self::billingSubject($subject)) {
            return self::Billing;
        ...
        if (preg_match('/по\s+принят\w+\s+(?:ТС|ГОТС)|по\s+при[её]му\s+(?:ТС|ГОТС)|принят[аоы]?\s+на\s+(?:хранение|парковку|стоянку)|фотоотч[её]т|^(?:ТС|ГОТС|машина|автомобиль)\s+принят[аоы]?\b|(?:ТС|ГОТС|машин[ау])\s+(?:привезли|доставили|сдали|приняли)\b/iu', $text)
