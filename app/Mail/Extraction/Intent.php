...
enum Intent: string
{
    ...
    public static function ofOutgoing(?string $subject, ?string $body, array $filenames = []): self
    {
        ...
        // «Фотоотчёт и документы по принятому ТС» — приём, хоть у СОГАЗа и файл «Апп» (акт приёма-передачи на хранение),
        // и «закрывающие документы по принятому ТС» тоже: без этой оговорки такие письма выдавали ТС (ReleaseByLetters).
        $intake = preg_match('/по\s+принят\w+\s+(?:ТС|ГОТС)|фотоотч[её]т/iu', $text) && ! preg_match('/подписанн\w+\s+АПП|по\s+выданн\w+/iu', $text);
        // Выдача — раньше бухгалтерии: СОГАЗу закрывающие документы по выданной ТС шлют в ветке «Стоянка ИП Кузнецов».
        if (! $intake && preg_match('/подписанн\w+\s+АПП|по\s+выданн\w+\s+(?:ТС|ГОТС)|закрывающие\s+документы/iu', $text)
