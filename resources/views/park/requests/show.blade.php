    ...
    <div class="flex flex-col gap-4">
        ...
        <x-ui.card>
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if ($req->planned_at)<span class="chip tabular-nums">{{ $req->planned_at->translatedFormat('j M, H:i') }}</span>@endif
            ...
            <form method="post" action="/zayavki/{{ $req->id }}/priem" id="act-form" class="flex flex-col gap-4">
                ...
                <x-ui.card title="Приём">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <x-ui.field name="damage_note" label="Что ещё заметили" type="textarea" class="sm:col-span-2"/>
                    </div>
                </x-ui.card>
                <x-ui.card title="Фото при приёме" data-controller="photos" data-photos-url-value="/mashiny/{{ $vehicle->id }}/media">
                    ...
                    <div class="mb-3"><x-ui.button type="button" variant="secondary" size="sm" data-action="photos#pick"><x-ui.icon name="camera" class="size-4"/> Снять</x-ui.button></div>
            ...
            <form method="post" action="/zayavki/{{ $req->id }}/vydacha" id="act-form">
                ...
                <x-ui.card title="Выдача">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <x-ui.field name="note" label="Кому, по какому документу" type="textarea" class="sm:col-span-2"/>
                    </div>
                </x-ui.card>
            </form>
            <x-ui.action-bar>
