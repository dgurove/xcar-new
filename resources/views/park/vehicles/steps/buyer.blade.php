...
@if (! $pass)
    <div class="list mt-2">
        <button type="button" class="pass-row w-full text-left" data-controller="copy" data-copy-text-value="{{ $vehicle->pickupUrl() }}" data-copy-done-value="Ссылка в буфере" data-action="copy#copy:prevent">
        ...
        @if ($canManage)
            <button type="submit" form="pickup-link-form" class="pass-row w-full text-left" data-turbo-confirm="Отправить страховой ссылку для покупателя ещё раз?"><span>Отправить страховой ещё раз</span><x-ui.icon name="send" class="size-5 shrink-0 text-ink-dim"/></button>
        @endif
    </div>
@else
    <div class="list mt-2">
        ...
        <div class="pass-row"><span class="nums shrink-0 whitespace-nowrap">{{ $pass->phone }}</span><span class="min-w-0 truncate text-ink-muted">{{ $pass->email }}</span></div>
        @if ($pass->requestMessage)
            <button type="button" class="pass-row w-full text-left" data-controller="emit" data-action="emit#send" data-emit-event-param="letters:open" data-emit-url-param="/cars/{{ $vehicle->id }}/letters">
