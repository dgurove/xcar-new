...
<div class="flex flex-col gap-2">
    @if ($debt['overdue'] > 0 || $debt['owed_to_us'] > 0 || $debt['we_owe'] > 0 || $unbilledTotal > 0)
        <div class="list">
            @if ($debt['overdue'] > 0)<div class="pass-row"><span class="text-danger">Просрочено</span><span class="nums font-semibold text-danger">{{ Money::rub($debt['overdue']) }}</span></div>@endif
            @if ($debt['owed_to_us'] > 0)<div class="pass-row"><span class="text-ink-muted">Нам должны</span><span class="nums font-semibold">{{ Money::rub($debt['owed_to_us']) }}</span></div>@endif
            @if ($debt['we_owe'] > 0)<div class="pass-row"><span class="text-ink-muted">Мы должны</span><span class="nums font-semibold text-urgent">{{ Money::rub($debt['we_owe']) }}</span></div>@endif
            @if ($unbilledTotal > 0)<div class="pass-row"><span class="text-ink-muted">Не выставлено</span><span class="nums">{{ Money::rub($unbilledTotal) }}</span></div>@endif
        </div>
    @endif
    ...
    @if ($toBill->isNotEmpty())
        ...
        <div class="list">
            @foreach ($toBill as $v)
                <a href="/cars/{{ $v->id }}/invoices/new" class="pass-row">
    ...
    @else
        <div class="list">
            @foreach ($invoices as $i)
                <a href="/money/invoices/{{ $i->id }}" class="pass-row">
