{{-- Предложения менеджеров в PDF: альбомный A4 по бренду — Onest, лайм, тонкие линии.
...
@php
    ...
    foreach ($tables as $t) {
        ...
        $head = ['ДЛ', 'Машина', 'Тип, город', 'Наша цена', 'Максимальная', 'Минимальная'];
        $car = fn ($r) => [$r[0], trim($r[1].' '.$r[2]).($r[3] ? ', '.$r[3] : ''), $r[4].($r[5] ? ', '.$r[5] : ''), $r[6], $r[7], $r[8]];
