        ...
        @foreach ($templates as $template)
            <a href="/settings/templates/{{ $template->id }}" class="row">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-1 flex min-w-0 items-center gap-2"><span class="shrink-0 text-sm text-ink-muted">{{ $template->scope->label() }}</span><span class="truncate text-sm text-ink-muted">{{ $template->subject }}</span></div>
                </div>
