...
<x-ui.shell title="Метки" :count="$tags->count()" narrow>
    <div class="flex flex-col gap-2" data-controller="order" data-order-url-value="/nastroyki/tegi/poryadok">
        @foreach ($tags as $tag)
            <div class="row" data-id="{{ $tag->id }}" data-controller="sheet">
                ...
                <x-ui.sheet id="tag-{{ $tag->id }}" title="Метка">
                    <form method="post" action="/nastroyki/tegi/{{ $tag->id }}" class="flex flex-col gap-4">
                        @csrf @method('put')
                    ...
                    <form method="post" action="/nastroyki/tegi/{{ $tag->id }}" class="mt-2" data-turbo-confirm="Удалить метку «{{ $tag->name }}»?{{ ($used[$tag->name] ?? 0) ? ' Она снимется с '.$used[$tag->name].' предложений.' : '' }}">
                        @csrf @method('delete')
    ...
    <div class="mt-4" data-controller="sheet">
        ...
        <x-ui.sheet id="tag-new" title="Новая метка">
            <form method="post" action="/nastroyki/tegi" class="flex flex-col gap-4">
                @csrf
