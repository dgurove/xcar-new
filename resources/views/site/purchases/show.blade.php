...
@endphp
<x-ui.shell :title="$purchase->publicTitle()" :heading="false" :trail="[['Главная', '/'], ['Закупки', '/zakupki'], [$purchase->publicTitle()]]">
    <div class="box">
        <h1 class="text-[26px] sm:text-[32px]">{{ $purchase->publicTitle() }}</h1>
    ...
    <x-ui.toolbar class="mt-5" :sorts="\App\Http\Site\PurchaseController::SORTS" :sort="$filters['sort'] ?? 'dl'" :pills="\App\Http\Site\PurchaseController::PRESETS" :pill="$filters['preset'] ?? 'all'" pill-param="preset" :hidden="['kind' => $filters['kind'] ?? null, \App\Support\ListView::PARAM => $view]" name="purchase">
    ...
    <div class="mt-6">
        @if ($cars->isEmpty())
            <x-ui.empty :href="'/zakupki/'.$purchase->number" link="Сбросить фильтры">По этим условиям ничего не нашлось.</x-ui.empty>
        @else
