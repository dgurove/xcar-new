...
<tr id="request-{{ $req->id }}" data-peek-url="/requests/{{ $req->id }}/peek" data-href="{{ $href }}" tabindex="0" class="{{ $req->isOverdue() ? 'text-danger' : '' }}">
    ...
    <td class="grow">{{ $v->titleWithYear() }}@if ($v->plate) <span class="nums hidden text-ink-muted sm:inline">{{ $v->plate }}</span>@endif
        @if ($todo = $req->todo())<span class="block max-w-[28rem] truncate text-xs text-ink-muted">{{ $todo }}</span>@endif</td>
