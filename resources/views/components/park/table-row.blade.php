...
<tr id="vehicle-{{ $vehicle->id }}" data-peek-url="{{ $href }}/peek" data-href="{{ $href }}" tabindex="0">
    ...
    <td class="text-[11px] sm:text-[13px]"><span class="dot {{ match ($state->tone()) { 'open' => 'dot-open', 'urgent' => 'dot-urgent', default => '' } }}"></span>{{ $state->label() }}@if ($state === VehicleState::Expected && $vehicle->relationLoaded('requests') && ! $vehicle->requests->contains(fn ($r) => $r->isOpen())) <span class="text-urgent">без заявки</span>@endif @if (! $state->isFinal() && $vehicle->noLetters()) <span class="tag tag-urgent">Писем нет</span>@endif @if ($vinProblem) <span class="tag tag-danger">{{ $vinProblem }}</span>@endif</td>
    <td class="hidden text-ink-dim sm:table-cell">@if ($noYard)<span class="tag tag-urgent">Не указана</span>@else<x-ui.place>{{ $vehicle->yard?->name }}</x-ui.place>@endif</td>
