...
@if ($vehicle->state === \App\Park\VehicleState::Stored)
    @if ($vehicle->yard)<x-ui.place class="chip">{{ $vehicle->yard->name }}{{ $vehicle->spot ? ', '.$vehicle->spot : '' }}</x-ui.place>@else<span class="chip bg-urgent-soft text-urgent">без парковки</span>@endif
