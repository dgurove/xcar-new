{{-- Состояние машины пилюлями: где стоит и сколько (светофор простоя), в пути сколько дней, когда выдана или почему не привезена.
     only — одна пилюля состояния, без места, дней и тегов ошибок данных: в почте рядом с названием машины
...
@endif
@if (! $only && ! $vehicle->state->isFinal() && $vehicle->noLetters())<x-ui.pill tone="urgent" class="!min-h-0 !py-1 text-xs">Писем нет</x-ui.pill>@endif
@if (! $only && ! $vehicle->state->isFinal() && $vehicle->vinProblem())<x-ui.pill tone="danger" class="!min-h-0 !py-1 text-xs">{{ $vehicle->vinProblem() }}</x-ui.pill>@endif
