{{-- Форма контакта: роль — только своей стороны; в CRM она одна («Реализация») и не выбирается, парковки у
     контакта продажи нет. --}}
@props(['contact', 'action', 'method' => 'post', 'sale' => false, 'yards' => []])
@php
    use App\Vendors\ContactRole;
    $roles = collect(ContactRole::side($sale));
@endphp
<form method="post" action="{{ $action }}" class="flex flex-col gap-4">
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        @if ($roles->count() > 1)
            <x-ui.field name="role" label="По какому вопросу" :options="$roles->mapWithKeys(fn ($r) => [$r->value => $r->label()])->all()" :value="$contact?->role->value ?? $roles->first()->value" span="col-span-2"/>
        @else
            <input type="hidden" name="role" value="{{ $roles->first()->value }}">
        @endif
