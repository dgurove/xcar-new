...
<form method="post" action="/offers/{{ $offer->number }}/izbrannoe" id="fav-{{ $offer->number }}{{ $variant === 'compact' ? '-compact' : '' }}" class="contents" data-queue data-controller="favorite" data-action="turbo:submit-start->favorite#press turbo:submit-end->favorite#settle" @guest data-turbo="false" @endguest>
    @csrf
    @if ($variant === 'compact')
