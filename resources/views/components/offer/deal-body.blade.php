...
@props(['offer', 'myBid' => null, 'myInterest' => null, 'chat' => null, 'chatsCount' => 0, 'inSheet' => false])
@php
    $user = auth()->user();
...
@if ($prices && $offer->asking_price)
    <div class="nums text-[32px] leading-none">{{ number_format($offer->asking_price, 0, '', ' ') }} ₽ @if ($offer->prices_include_vat)<span class="text-sm font-normal text-ink-muted">с НДС</span>@endif</div>
...
@else
    ...
    @if ($user->isStaff())
        <a href="/admin/offers/{{ $offer->number }}" class="btn btn-quiet mt-6 w-full">Редактировать</a>
        @if ($chatsCount)<a href="/admin/chaty?q={{ $offer->number }}&preset=all" class="btn btn-quiet mt-3 w-full"><x-ui.icon name="chat" class="size-5"/> Чаты · {{ $chatsCount }}</a>@endif
    @elseif ($chat)
