{{-- Таблица списка (третий вид): плотные строки, по нажатию строка выделяется и
     ...
     ↑/↓ — по строкам; open — id строки, которую открыть сразу (?peek=). --}}
@props(['head', 'id' => null, 'open' => null])
<div {{ $attributes->merge(['class' => 'table-wrap']) }} data-controller="peek" data-action="keydown->peek#key turbo:frame-load->peek#loaded turbo:before-fetch-request->peek#request turbo:click->peek#leave" @if ($open) data-peek-open-value="{{ $open }}" @endif @if ($id) id="{{ $id }}" @endif>
    <div class="table-box">
        <table class="table">
            <thead>{{ $head }}</thead>
