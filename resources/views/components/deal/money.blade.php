{{-- Деньги сделки для сотрудника: цена, закупочная снимком, разница, агентское вознаграждение с режимом и
     состоянием (правится, пока нет счёта), «нам»; счета со светофором, заявленные оплаты с решением,
     вознаграждение к выплате с «Выплатить». Менеджеру ничего из этого не показывается. --}}
@props(['deal'])
@php
...
<x-ui.card title="Деньги" {{ $attributes }}>
    <dl class="grid grid-cols-[auto_1fr] items-baseline gap-x-4 gap-y-1.5">
        ...
        <dt class="text-sm text-ink-dim">Агентское вознаграждение</dt>
        <dd class="flex flex-wrap items-center justify-end gap-1.5"><span class="nums">{{ $deal->commission ? Money::rub($deal->commission) : 'нет' }}</span>
            @if ($deal->commission)<span class="tag">{{ mb_strtolower($deal->commission_mode->label()) }}</span>@endif
            @if ($state !== CommissionState::Hidden)<x-ui.pill :tone="$state->tone()" class="!min-h-0 !py-1 text-xs">{{ $state->label() }}{{ $state === CommissionState::Payable && $fee ? ' до '.$fee->due_at->translatedFormat('j M') : '' }}{{ $state === CommissionState::Paid && $fee?->paid_at ? ' '.$fee->paid_at->translatedFormat('j M') : '' }}</x-ui.pill>@endif
        </dd>
        ...
        @if ($deal->isActive())
            @if ($deal->commissionEditable())
                <div data-controller="sheet" class="contents">
                    <button type="button" class="chip" data-action="sheet#open">Изменить вознаграждение</button>
            ...
            @foreach ($invoices as $i)
                <div class="row !py-2.5 {{ $i->claims->isNotEmpty() ? 'bg-urgent-soft' : '' }}">
                    <a href="/work/money/invoices/{{ $i->id }}" class="min-w-0 flex-1">
                        <span class="block truncate">{{ $i->isOwed() ? 'Вознаграждение' : 'Счёт '.$i->label() }}<span class="text-ink-muted"> {{ $i->party->name }}</span></span>
                ...
                @if ($i->isAgentFee() && $i->state === InvoiceState::Issued)
                    <div class="pl-3" data-controller="sheet">
