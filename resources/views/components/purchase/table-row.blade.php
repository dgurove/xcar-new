{{-- Строка таблицы машин закупки (CRM): ДЛ, машина, тип, размещение, предложения
     менеджеров числом и лучшей, наша цена, флаги. Нажатие — окошко (peek), в нём и оценка;
...
<tr id="car-{{ $car->id }}" data-peek-url="{{ $href }}/peek" data-href="{{ $href }}" tabindex="0" class="{{ $car->is_published ? '' : 'text-ink-muted' }}" @if (!$car->price_final) data-unpriced @endif>
    ...
    <td class="num nums">@if ($car->price_final)<span class="font-bold">{{ \App\Support\Money::nums($car->price_final) }}</span>@else<span class="text-accent-text">оценить</span>@endif</td>
