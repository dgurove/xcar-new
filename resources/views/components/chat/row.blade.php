...
<a href="{{ $href }}" class="chat-row" @if ($current) aria-current="true" @endif data-turbo-action="advance">
    @if ($chat->offer)
        <div class="chat-row-photo">
            ...
            @if ($other || !$staff)<x-chat.avatar :user="$other" :size="22" class="chat-row-avatar"/>@endif
        </div>
    @elseif ($other || !$staff)
        <x-chat.avatar :user="$other" :size="44"/>
    @else
    ...
    <div class="min-w-0 flex-1">
        ...
        @if ($chat->offer || $staff)<div class="truncate text-[13px] text-ink-muted">{{ $chat->offer ? $chat->offer->titleWithYear() : 'Обращение с сайта' }}</div>@endif
