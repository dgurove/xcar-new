<x-ui.auth title="Вход">
    ...
    <form method="post" action="/vhod" class="mt-6 space-y-3">
        @csrf
        <input name="login" type="text" required autocomplete="username" inputmode="email" class="field-input" placeholder="Телефон или почта" value="{{ old('login') }}" autofocus>
    ...
    <div data-controller="passkey" data-passkey-mode-value="login" hidden data-passkey-target="root">
        ...
        <button type="button" class="btn btn-quiet w-full" data-action="passkey#login"><x-ui.icon name="faceid" class="size-5"/> Войти по Face ID</button>
    </div>
